...
function updateTargetBluePoints() {
  // KORRIGIERT: Korrekte Ziel-Werte für alle Ziffern
  digits[0].targetBluePoints = h; // h10 - Stunden (0-23)
  digits[1].targetBluePoints = h; // h1 - Stunden (0-23)
  digits[2].targetBluePoints = m; // m10 - Minuten (0-59) KORRIGIERT!
  digits[3].targetBluePoints = s; // m1 - Sekunden (0-59)
}
...
function generateDigitGrid(centerX, centerY) {
  // Erstelle ein regelmäßiges 12×5 Grid (60 Punkte)
  let points = [];
  
  // Startposition für das Grid (oben links)
  let startX = centerX - DIGIT_WIDTH/2 + POINT_SPACING_X;
  let startY = centerY - DIGIT_HEIGHT/2 + POINT_SPACING_Y;
  
  for (let row = 0; row < GRID_ROWS; row++) {
    for (let col = 0; col < GRID_COLS; col++) {
      let x = startX + col * POINT_SPACING_X;
      let y = startY + row * POINT_SPACING_Y;
      points.push({ x: x, y: y });
    }
  }
  
  // Mischen für zufällige Füll-Reihenfolge
...
function drawDigit(digit) {
  ...
  digit.targets.forEach((target, index) => {
    if (index < digit.bluePoints) {
      // Blauer Punkt (ersetzt) - statisch
      fill(...COLORS.dynamic);
    } else {
      // Grauer Punkt (original)
      fill(COLORS.static);
    }
