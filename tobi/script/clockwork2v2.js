...
function drawWaterWithHourWaves(level, hours) {
  ...
  // Device-Einfluss berechnen
  let deviceAmplitude = map(tiltX, -90, 90, -20, 20); // Kippt vor/zurück
  let deviceOffset = map(tiltY, -90, 90, -PI / 3, PI / 3); // Kippt seitlich

  for (let x = 0; x <= width; x++) {
    let baseWave = sin((x / waveLength) * TWO_PI + frameCount * waveSpeed + deviceOffset) * (waveAmplitude + deviceAmplitude);
...
// 📱 Device-Bewegung erfassen
function deviceMoved() {
  tiltX = rotationX;
  tiltY = rotationY;
}

// 📲 Erlaube Zugriff auf Bewegungssensor bei iOS
